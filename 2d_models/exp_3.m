...
% Stress-constrained 2D backslip model
% Minimum coupling (sum penalty 100)
...
userParams.datasetFilenames = {'test_data_2d/gps_profile.dat'};
%
...
userParams.lsqlinOptions = optimoptions('lsqlin','MaxIter',1e5,'display','off');
...
userParams.smoothingType = {{ {'stressKernel', 0}, {'sum', 'dip', 100} }}; % need one cell list for each source. Current options: {'stressKernel', 0}, {'sum', 'component', target_sliprate}
userParams.smoothingWeights = {{ 1e-3, 0 }};
userParams.stressKernelFolder = 'test_data_2d';
userParams.constraintType = {{ {'positiveStress', 25e3, 100/cosd(10)} }}; % Current options: {'positiveStress',depth, Vds}
