function params = exp_101()
    ...
    params.dataComponents = [1]; %here we keep only [East] comoponents of the data. Options: single component, or list from [1,2,3]. Default: all 3
    ...
    params.sourceFilenames  = {'faults/ramp_2d.seg'};
    ...
    params.smoothingType = {{ {'laplacian_1d', 0} }}; % need one cell list for each source. Current options: {'laplacian_1d', 0}, {'laplacian', 0}, {'stressKernel', 0}, {'sum', 'component', target_sliprate}
    %params.nNearest = 2; % for Laplacian smoothing - how many nearest neighbors to include. Standard is 4, but any number can be used - distance weighting is applied
    params.smoothingWeights = {{ 1 }};
