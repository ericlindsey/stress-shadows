...
userParams.dataComponents = [1,2]; %here we keep only [East,North] comoponents of the data. Options: single component, or list from [1,2,3]. Default: all 3
...
%userParams.slipComponents = [1,2]; % list of components to keep - either 1 (strike), 2 (dip), or both [1,2]. Default: both
...
userParams.bounds = {{ {'dip', 0, 100/cosd(10)}, {'strike', -50, 50} }}; %currently defined as one cell list for dip and strike variables (e.g. {'dip', min, max}), with one pair for each source.

userParams.smoothingType = {{ {'stressKernel', 0}, {'sum','dip', 0} }}; % need one cell list for each source. Current options: {'stressKernel',0}, {'sum','component',target_sliprate}
...
userParams.constraintType = {{ {'positiveStress', 300e3, 0} }}; % Current options: {'positiveStress',depth, Vds}
