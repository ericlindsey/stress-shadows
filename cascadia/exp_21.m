function userParams = exp_21()

% This script configures jointinv2 for:
% - Cascadia inversion with backslip
% - No stress constraints
% - Rake coordinates with bounds on rake direction, slip bonus on rake
% - Michel data with Schmalzle rake model and block corrections
...
userParams.smoothingType = {{ {'laplacian', 0}, {'sum', 'rake', -1 } }}; %, {'value', 'rakePerp', 0} }}; % {'value', 'rakePar', 0}, {'value', 'rakePerp', 0} }}; % , {'sum','strike', 0}  }}; % need one cell list for each source. Current options: {'stressKernel',0}, {'sum','component',target_sliprate}
userParams.smoothingWeights = {{ 0.6702, 10^-3.05 }};
