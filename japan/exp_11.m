function userParams = exp_11()

% This script configures jointinv2 for:
% - Japan inversion with backslip
% - Long fault
% - No stress constraints
% - Rake coordinates with bounds on rake direction, slip penalties on rake (Vpl)
...
userParams.smoothingType = {{ {'laplacian', 0}, {'sum', 'rake', -1 } }}; %, {'value', 'rakePerp', 0} }}; % {'value', 'rakePar', 0}, {'value', 'rakePerp', 0} }}; % , {'sum','strike', 0}  }}; % need one cell list for each source. Current options: {'laplacian',N_nearest}, {'stressKernel',0}, {'sum','component',target_sliprate}
userParams.smoothingWeights = {{ 0.0947, 3.56e-4}};
