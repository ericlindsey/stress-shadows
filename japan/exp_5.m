...
userParams.datasetFilenames = {'gps_data/land_gps_tohoku_backslip_long.dat'};
%
...
userParams.sourceFilenames  = {'Slab2/japan_trench_xyz'};
%
...
userParams.bounds = {{ {'rakeSlipRate'}, {'rakePerp', -10, 10} }}; % %currently defined as one cell list for dip and strike variables (e.g. {'dip', min, max}), with one pair for each source.

userParams.smoothingType = {{ {'laplacian', 0}, {'sum', 'both', -100 } }}; %, {'value', 'rakePerp', 0} }}; % {'value', 'rakePar', 0}, {'value', 'rakePerp', 0} }}; % , {'sum','strike', 0}  }}; % need one cell list for each source. Current options: {'laplacian',N_nearest}, {'stressKernel',0}, {'sum','component',target_sliprate}
...
userParams.constraintType = {{ {'positiveStress', 40e3, 'rake'} }}; % Current options: {'positiveStress',depth, Vds}
...
userParams.rakeFile = 'Slab2/rake_PA_OK_kreemer_long.dat';
