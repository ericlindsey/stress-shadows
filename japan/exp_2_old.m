function userParams = exp_2()

% This script configures jointinv2 for:
% - Japan inversion with backslip
% - No stress constraints
% - Strike/Dip coordinates with constraints on rake direction, slip penalties on both
...
userParams.smoothingType = {{ {'stressKernel', 0}, {'value', 'both', 0 } }}; %, {'value', 'rakePerp', 0} }}; % {'value', 'rakePar', 0}, {'value', 'rakePerp', 0} }}; % , {'sum','strike', 0}  }}; % need one cell list for each source. Current options: {'stressKernel',0}, {'sum','component',target_sliprate}
userParams.smoothingWeights = {{ 1e-2, 1e-2 }};
