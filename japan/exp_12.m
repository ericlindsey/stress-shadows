...
% - Rake coordinates with bounds on rake direction, slip penalties on rake (0)
...
userParams.bounds = {{ {'rakeSlipRate'}, {'rakePerp', -9, 9} }}; %currently defined as one cell list for dip and strike variables (e.g. {'dip', min, max}), with one pair for each source.

userParams.smoothingType = {{ {'laplacian', 0}, {'sum', 'rake', 0 } }}; %, {'value', 'rakePerp', 0} }}; % {'value', 'rakePar', 0}, {'value', 'rakePerp', 0} }}; % , {'sum','strike', 0}  }}; % need one cell list for each source. Current options: {'laplacian',N_nearest}, {'stressKernel',0}, {'sum','component',target_sliprate}
userParams.smoothingWeights = {{ 0.0947, 3.56e-9}};
userParams.stressKernelFolder = 'kernels';
userParams.nNearest = 3; % for finite difference laplacian. tri:4 rect:5
userParams.constraintType = {{ {'positiveStress', 300e3, 'rake'} }}; % Current options: {'positiveStress',depth, Vds}
...
userParams.rakeFile = 'Slab2/rake_PA_OK_kreemer.dat';
