...
% (Kirkpatrick et al., 1983) [1]
%
% INPUT:
%        f      function handle
%        x0     initial guess for the minimun
%        l      lower bound for solution
%        u      upper bound for solution
%        Mmax   maximun number of temperature changes
%        TolFun tolerance of the function
%
% OUTPUT:
%        x0     global minimun
%        f0     f(x0)
...
%
% AUTHOR:
%    Héctor Corte (cortehector@uniovi.es)
...
for m=0:Mmax
    ...
    tstart=tic;
    if 0~=telapsed
        printRemainingTime(m,Mmax,(Mmax-m+1)*telapsed);
    else
        fprintf('iteration %d/%d\n',m+1,Mmax+1);
    end
    % For each temperature we take 500 test points to simulate reach thermal
    % equilibrium.
    for k=0:30
        % We generate new test point using mu_inv function [3]
        dx=mu_inv(2*rand(size(x))-1,T).*(u-l)/2;
        x1=x+dx;
        ...
        % point. If df > 0 we use Metropolis [5] condition to accept or
...
function x = mu_inv(y,T)
% function MU_INV is used to generate new point according to lower and upper
% and a random factor proportional to current point.
mu=10^(T*10);
%x=(((1+mu).^abs(y)-1)/mu).*sign(y);
x=(((1+mu).^abs(y)-1)/mu).*sign(y).*(1-0.9*T);
end

function [] = printRemainingTime(m,Mmax,telapsed)
remainingTime=(Mmax-m+1)*telapsed;
rth=floor(remainingTime/3600);
rtm=floor((remainingTime-rth*3600)/60);
rts=floor((remainingTime-rth*3600-rtm*60));
fprintf('iteration %d/%d, remaining time: %.1fh %.1fm %.1fs\n',...
    m+1,Mmax+1,rth,rtm,rts);
end
